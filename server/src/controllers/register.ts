...
function UserNameRegex(username: string): boolean {
...
export default class Register {
  ...
  static async signup(
    ...
    try {
      ...
      /*  else if (csrfToken !== req.csrfToken())
        return res.status(400).json(message("Request not valid"));*/ else if (
        firstName.length < 4 ||
        lastName.length < 4
      )
      ...
      else if (EmailExist) return res.status(302).json(message("Email exists"));
      ...
      let MailSent = await SendMail(
        email,
        `${process.env.FRONTENDURL!}/register/token?token=${JwtToken}`
      );
      res
        .status(300)
        .json(
  ...
  static async login(
    ...
    try {
      ...
      if (!isValid) return res.status(302).json(message("Incorrect password"));
      else if (isEmailValid.status === "Pending")
        return res
          .status(300)
          .json(
            message(
              "Please, click the link in your email to confirm your email"
            )
          );
      else {
        let token = jwt.sign(
  ...
  static async emailExists(
    ...
    try {
      ...
      if (!EmailRegex(email))
        return res.status(302).json(message("Invalid email format"));
      else if (isMailExist) return res.status(302).json(message("Email exist"));
  ...
  static async token(
    ...
    try {
      let payload = jwt.verify(token, process.env.SECRET_KEY!) as IPayload;
      console.log(payload);
